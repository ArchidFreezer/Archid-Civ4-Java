...
public class DefaultResourceArrayTagProcessor extends AbstractTagProcessor {
	...
	public String getExporterCellWriter() {
		...
		sb.append(NEWLINETT + "for (" + tagVals.wrapper + " wrapper: list.get" + tagVals.wrapper + "List()) {");
		sb.append(NEWLINETTT + "if (currHeight > 0) cellvalue.append(IInfoWorkbook.CELL_NEWLINE);");
		sb.append(NEWLINETTT + "cellvalue.append(wrapper.getResource() + IInfoWorkbook.CELL_NEWLINE);");
		sb.append(NEWLINETTT + "for (Integer element: wrapper.getElements()) {");
		sb.append(NEWLINETTTT + "cellvalue.append(element + IInfoWorkbook.CELL_NEWLINE);");
