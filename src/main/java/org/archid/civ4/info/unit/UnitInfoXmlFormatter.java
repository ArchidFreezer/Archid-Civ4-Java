...
public class UnitInfoXmlFormatter extends AbstractXmlFormatter {
	...
	public void format(String path) {
		...
		Map<String, Map<Integer, String>> navalCombats = new HashMap<String, Map<Integer, String>>();
		...
		for (IXmlInfo info: navalUnits) {
			...
			if (defaultUnit) {
				if (!navalCombats.containsKey(combatType)) navalCombats.put(combatType, new TreeMap<Integer, String>());
				Integer strength = Integer.parseInt(info.getTagValue(TAG_COMBAT));
				navalCombats.get(combatType).put(strength, unitClass);
		...
		for (String group: combatMap.keySet()) {
			...
			for (IXmlInfo info: combatInfos) {
				...
				if (!headerPrinted) {
					groupSB.append(newline + newline + groupHeader.replaceAll("xxxGROUPxxx",	getCommentText(group)));
					headerPrinted = true;
				}
